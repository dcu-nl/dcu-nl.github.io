---
permalink: /partners/
title: "Partners"
last_modified_at: 2026-03-17
---

<section class="section-intro">
	<p>
		The Dosimetry Core Unit collaborates with clinical and academic institutions to strengthen
		translational dosimetry research, harmonize methods, and support implementation in practice.
	</p>
</section>

<div class="content-grid">
	<article class="content-card">
		<h3>Radboud University Medical Center</h3>
		<p>Clinical and translational collaboration in molecular imaging and radionuclide therapy.</p>
		<p><a href="https://www.radboudumc.nl/en/research/at-departmental-level/medical-imaging" target="_blank" rel="noopener noreferrer">Visit RadboudUMC</a></p>
	</article>

	<article class="content-card">
		<h3>Erasmus Medical Center</h3>
		<p>Partnership in radiology and nuclear medicine research with a focus on patient impact.</p>
		<p><a href="https://www.erasmusmc.nl/en/research/departments/radiology-and-nuclear-medicine" target="_blank" rel="noopener noreferrer">Visit ErasmusMC</a></p>
	</article>

	<article class="content-card">
		<h3>Delft University of Technology</h3>
		<p>Technical and methodological collaboration through the Reactor Institute and radiation science groups.</p>
		<p><a href="https://www.tudelft.nl/tnw/over-faculteit/afdelingen/radiation-science-technology/research/research-groups/applied-radiation-isotopes" target="_blank" rel="noopener noreferrer">Visit TU Delft</a></p>
	</article>

	<article class="content-card">
		<h3>University Medical Center Groningen</h3>
		<p>Joint efforts in nuclear medicine and molecular imaging for advanced dosimetry applications.</p>
		<p><a href="https://umcgresearch.org/w/nuclear-medicine-and-molecular-imaging" target="_blank" rel="noopener noreferrer">Visit UMCG</a></p>
	</article>

	<article class="content-card">
		<h3>Netherlands Cancer Institute</h3>
		<p>Collaboration with the Netherlands Cancer Institute to strengthen cancer-focused research and translational impact.</p>
		<p><a href="https://www.nki.nl/research/research-groups/regina-beets-tan/" target="_blank" rel="noopener noreferrer">Visit NKI</a></p>
	</article>
</div>
